<% if $Image %><div class="image-banner--image {$ImageHeight}<% if $ImageOverlay != 'none' %> $ImageOverlay<% end_if %>" style="background-image: url('{$Image.URL}');" data-height="<% if $ImageHeight == 'bh-large' %>100<% end_if %><% if $ImageHeight == 'bh-medium' %>80<% end_if %><% if $ImageHeight == 'bh-small' %>40<% end_if %>"></div><% end_if %>
